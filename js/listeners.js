...
if (_chatSettingsEl) _chatSettingsEl.addEventListener('click', () => {
    ...
    setupAvatarFrameSettings();
            ...
            // 心意相通 - 查看时间
            const qViewMinSlider = document.getElementById('questionnaire-view-min-slider');
            const qViewMinValue = document.getElementById('questionnaire-view-min-value');
            const qViewMaxSlider = document.getElementById('questionnaire-view-max-slider');
            const qViewMaxValue = document.getElementById('questionnaire-view-max-value');

            function updateQViewUI() {
                if (qViewMinSlider) qViewMinSlider.value = settings.questionnaireViewMin;
                if (qViewMinValue) {
                    const minSec = settings.questionnaireViewMin / 1000;
                    qViewMinValue.textContent = minSec >= 60 ? `${(minSec/60).toFixed(1)}分钟` : `${minSec.toFixed(0)}s`;
                }
                if (qViewMaxSlider) qViewMaxSlider.value = settings.questionnaireViewMax;
                if (qViewMaxValue) {
                    const maxSec = settings.questionnaireViewMax / 1000;
                    qViewMaxValue.textContent = maxSec >= 60 ? `${(maxSec/60).toFixed(1)}分钟` : `${maxSec.toFixed(0)}s`;
                }
            }
            updateQViewUI();

            if (qViewMinSlider) {
                qViewMinSlider.addEventListener('input', (e) => {
                    let val = parseInt(e.target.value, 10);
                    if (val > settings.questionnaireViewMax) {
                        val = settings.questionnaireViewMax;
                        e.target.value = val;
                    }
                    settings.questionnaireViewMin = val;
                    updateQViewUI();
                });
                qViewMinSlider.addEventListener('change', throttledSaveData);
            }

            if (qViewMaxSlider) {
                qViewMaxSlider.addEventListener('input', (e) => {
                    let val = parseInt(e.target.value, 10);
                    if (val < settings.questionnaireViewMin) {
                        val = settings.questionnaireViewMin;
                        e.target.value = val;
                    }
                    settings.questionnaireViewMax = val;
                    updateQViewUI();
                });
                qViewMaxSlider.addEventListener('change', throttledSaveData);
            }
